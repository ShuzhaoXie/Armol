...
from src.common import CUR_DEVICE, TEST_RANK, Detections, json_load, TEST_IMAGE_DIR, ONLINE, to_numpy, ALPHA
...
class ARMOR_TEST(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self):
        super(ARMOR_TEST, self).__init__()
        ...
        self.t = 0
        self.gt = Detections('gt', TEST_RANK)
        self.aws = Detections('aws', TEST_RANK)
        self.azure = Detections('azure', TEST_RANK)
        self.google = Detections('google', TEST_RANK)
        # self.best_actions = parse_pattern(WORK_DIR + '/dds/src/upper_bound_{}_pattern.json'.format(box_fusion_mode))
        self.image_rank = TEST_RANK
        self.image_path = os.path.join(TEST_IMAGE_DIR, self.image_rank[self.t])
        self.nms_mode = wbf
    ...
    def _get_next_state(self):
        image_name = self.image_rank[self.t]
        img = Image.open(os.path.join(TEST_IMAGE_DIR, image_name)).convert('RGB')
        tensor = transform(img).unsqueeze(0).to(CUR_DEVICE)
        with torch.no_grad():
            out = model(tensor)
        probabilities = torch.nn.functional.softmax(out[0], dim=0)
