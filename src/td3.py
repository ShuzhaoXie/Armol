...
from common import get_top_k, write_results_to_file
import logging
import src.td3_core as core
from src.common import CUR_DEVICE, cal_mAP, get_top_k, write_results_to_file, to_tensor, to_numpy, check_exist_dir_path
...
def td3(train_env_fn, test_env_fn, logger_2_file, tmp_predict, wolp=1, cur_device = CUR_DEVICE,
    ...
    # Experience buffer
    replay_buffer = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size, device=cur_device)
    ...
    def compute_loss_q(data):
        ...
        loss_info = dict(Q1Vals=to_numpy(q1),
                         Q2Vals=to_numpy(q2))
    ...
    def get_action(o, noise_scale):
        a = ac.act(o)
        a += noise_scale * to_tensor(np.random.randn(act_dim))
        return torch.clamp(a, 0, act_limit)

    def wolper(a):
        k = wolp
        topk_actions = get_top_k(a, k)
        
        with torch.no_grad():
            os = torch.ones((k, 1000), device=cur_device).mul(o)
            # print(os.shape, topk_actions.shape)
            qv1 = ac.q1(os, topk_actions)
            qv2 = ac.q2(os, topk_actions)
            qv_min = torch.min(qv1, qv2)
            top1_qv, top1_id = torch.topk(qv_min, 1)
            
            return topk_actions[top1_id][0]
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            cost_all = 0
            actions = []
            sum_action = np.array([0., 0., 0.])
            check_exist_dir_path(tmp_predict)

            while not(d or (ep_len == max_ep_len)):
                # Take deterministic actions at test time (noise_scale=0)
                a = wolper(get_action(o, 0))
